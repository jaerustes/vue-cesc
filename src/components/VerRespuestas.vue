<template>
  <div class="container mt-5 d-flex justify-content-center">
    <div
      class="card p-4 bg-light border-primary shadow-lg"
    ...
    >
      ...
      <div class="mb-3">
        <label class="form-label fw-bold text-primary"
          >Selecciona un sociograma:</label
        >
        <select
      ...
      <div v-if="protocoloSeleccionado" class="mt-3 text-primary">
        <p>
          <strong>Alumnes que han acabat:</strong> {{ respondidos }} /
          {{ totalAlumnos }}
        </p>
      ...
      <div v-if="respondidos > 0" class="mt-3">
        ...
        <ul class="list-group">
          <li
            v-for="(respuesta, alumnoId) in respuestas"
            :key="alumnoId"
...
export default {
  ...
  setup() {
    ...
    const cargarRespuestas = async () => {
      ...
      const protocoloDoc = await getDoc(
        doc(db, 'protocolos', protocoloSeleccionado.value),
      )
      ...
      unsubscribe = onSnapshot(respuestasRef, snapshot => {
        ...
        snapshot.forEach(doc => {
          respuestas.value[doc.id] = {
            ...
            nombre:
              doc.data().nombre || `Alumno ${doc.id.replace('alumno_', '')}`,
          }
        })
